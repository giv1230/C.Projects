...
int getCommand(FILE * source, char * command) {
    if (source == stdin) {  // if the file is stdin then just print # and then pint the command to the screen
        printf("# ");
        fflush(stdout);
    }

    if (!fgets(command, MAX_COMMAND_LEN, source)) {    // write the command in to the file source and then start a new line
...
/*************************************************************************************/
        ...
        for(job = jobList->head; job->next; job = job->next){
            statusString = (jobList.head->progs[j].isStopped)? tr:fl; 
            printf(JOB_STATUS_FORMAT, jobList.head->jobId, statusString,jobList.head->text);
        }
        return 0;
/**************************************************************************************/
         ...
         // FILL IN HERE
        char *check1;  // This is syntax checking
        if(newJob.numProgs == 2 && (newJob.progs[1].argv[0] == '%')){
            check1 = newJob.progs[1].argv;
            check1++;
            if(!isdigit(check1)) return 1;
            while(check1 != '\0' || check1 != '\n' || check1 != EOF){
                if(isdigit(check1)) check1++;
                else{
                    return 1;
                }
            }
        ...
        j = 0;
        int commnum = 0;
        check1 = newJob.progs[1].argv[1]; // here we get the jobId 
        while(check1[j] != '\0' || check1[j] != '\n' || check1[j] != EOF){
            if(check1[j] >= '0' && check1[j] <= '9'){
                commnum = commnum*10;
                commnum = commnum + (check1[j] - '0');
            }
        }
        struct job *h;
        h = jobList.head;
        ...
        j = 1;
        while(h.jobId != commnum || h.jobId != EOF){  // Looking for the job in the jobList
            h = h->next;
            if(j == 0) job = job->next; 
            if(j == 1) j == 0;
        }
        ...
        if(h.jobId == commnum){
            If (strcmp(newJob.progs[0].argv[0], "fg") == 0){
                if(tcsetpgrp(0 , h.pgrp)) perror("tcssetpgrp");
                else{
                    h.stoppedProgs = 0;
                    while(j < h.numProgs) {
                        h.progs[j]->isStopped = 0;
                        j++;
                    }
                }
                for(job1 = jobList->fg; job1->next; job1 = job1->next);
                job1->next = h;
                job->next = h.next;
                h.next = NULL;
                kill(h->pgrp, SIGCONT);
	...
	// then put the job you found in the foreground (use tcsetpgrp(ps))
